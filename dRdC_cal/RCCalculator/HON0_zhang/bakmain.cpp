...
int main(int argv, char *argc[])
{
	...
	*/
	...
	frequencyAll=(double*)malloc(sizeof(double)*21);
	for(i=0;i<seqNum;i++) frequency[i]=(double*)malloc(sizeof(double)*20);
	...
	for(i=0;i<seqNum;i++) {
		...
		for(j=0;j<seqLength;j++) {
			SS[i][j]=synonymous(ratio,sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2], frequencyAll);
			S[i]=S[i]+SS[i][j];
			NN[i][j]=3.0-SS[i][j];
	...
	for(i=0;i<seqNum-1;i++) {
		for(j=i+1;j<seqNum;j++) {
			...
			if (Dc == 0) {
				DrDcRatio = 1.0;printf("WARNING : %d %d has Consertive Change=0, set Dr/Dc=1", i, j);
			}
				...
				if(a4==3) {
					...
					if(aa(x1,x2,x3)==20||aa(y1,y2,y3)==20) {
                        ...
                        wt[0] = ComputeQab(sequenceN[i][k][0], sequenceN[i][k][1], sequenceN[i][k][2], 
                            ...
                            ComputeQab(y1,y2,y3, 
                            sequenceN[j][k][0], sequenceN[j][k][1], sequenceN[j][k][2], frequencyAll[aa(sequenceN[i][k][0], sequenceN[i][k][1], sequenceN[i][k][2])], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp);
                    }
					x1=sequenceN[i][k][0];
					x2=sequenceN[i][k][1];
					x3=sequenceN[j][k][2];
...
{
	double x,y,count, x1, y1;
	int flag,flag1,i,j,k,k0,u,v,w;
	x=ratio/(ratio+2.0)*frequencyAll[aa(u0,v0,w0)]; //weight of codon A
	y=1.0/(ratio+2.0)*frequencyAll[aa(u0,v0,w0)];
	x1 = ratio/(ratio+1.0)*frequencyAll[aa(u0,v0,w0)];
	y1 = 1.0/(ratio+1.0)*frequencyAll[aa(u0,v0,w0)];
	count=.0;
	u=u0; v=v0; w=w0;
	flag=0; flag1=0;
	K = ratio*2;
	...
	for(i=1;i<=3;i++) {
		...
		if((indexs[aa(u,v0,w0)]!=indexs[aa(u0,v0,w0)])&&(indexs[aa(u,v0,w0)]!=-1)) {
			if(flag==0) {
				if((u0==0&&u==1)||(u0==1&&u==0)||(u0==2&&u==3)||(u0==3&&u==2)) count=count+x*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
				}
			}
			if(flag==2) {
				count=count+1;
			...
			if(flag==1&&flag1==0) {
				if((u0==0&&u==1)||(u0==1&&u==0)||(u0==2&&u==3)||(u0==3&&u==2)) count=count+x1*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
				}
			}
	...
	for(i=1;i<=3;i++) {
		...
		if((indexs[aa(u0,v,w0)]!=indexs[aa(u0,v0,w0)])&&(indexs[aa(u0,v,w0)]!=-1)) {
			if(flag==0) {
				if((v0==0&&v==1)||(v0==1&&v==0)||(v0==2&&v==3)||(v0==3&&v==2)) count=count+x*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
				}
			}
			...
			if(flag==1&&flag1==0) {
				if((v0==0&&v==1)||(v0==1&&v==0)||(v0==2&&v==3)||(v0==3&&v==2)) count=count+x1*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y1*frequencyAll[aa(u,v0,w0)];
				}
			}
		}
	}
	...
	for(i=1;i<=3;i++) {
		...
		if((indexs[aa(u0,v0,w)]!=indexs[aa(u0,v0,w0)])&&(indexs[aa(u0,v0,w)]!=-1)) {
			if(flag==0) {
				if((w0==0&&w==1)||(w0==1&&w==0)||(w0==2&&w==3)||(w0==3&&w==2)) count=count+x*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
				}
			}
			...
			if(flag==1&&flag1==0) {
				if((w0==0&&w==1)||(w0==1&&w==0)||(w0==2&&w==3)||(w0==3&&w==2)) count=count+x1*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y1*frequencyAll[aa(u,v0,w0)];
				}
			}
...
double conserve(double ratio, double KaKsRatio, int u0, int v0, int w0, double* frequencyAll)
{
	...
	for(i=1;i<=3;i++) {
		v=v+1;
		if(v==4) v=0;
		if((aa(u,v0,w0)!=aa(u0,v0,w0))&&(indexs[aa(u0,v,w0)]==indexs[aa(u0,v0,w0)])&&(indexs[aa(u0,v,w0)]!=-1)) {
			if(flag==0) {
				if((v0==0&&v==1)||(v0==1&&v==0)||(v0==2&&v==3)||(v0==3&&v==2)) count=count+x*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
	...
	for(i=1;i<=3;i++) {
		w=w+1;
		if(w==4) w=0;
		if((aa(u,v0,w0)!=aa(u0,v0,w0))&&(indexs[aa(u0,v0,w)]==indexs[aa(u0,v0,w0)])&&(indexs[aa(u0,v0,w)]!=-1)) {
			if(flag==0) {
				if((w0==0&&w==1)||(w0==1&&w==0)||(w0==2&&w==3)||(w0==3&&w==2)) count=count+x*K*frequencyAll[aa(u,v0,w0)];
				else {
					count=count+y*frequencyAll[aa(u,v0,w0)];
