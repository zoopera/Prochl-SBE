...
int main(int argv, char *argc[])
{
	...
	*/
	...
	frequencyAve=(double*)malloc(sizeof(double)*70);
	...
	for(i=0;i<seqNum;i++) {
		for(j=0;j<70;j++) {
			frequency[i][j]=1.0; /*changed by JT*/
		frequencyAllCodon[i][j] = 1.0; /*changed by JT*/
		}
	}
	for(i=0;i<70;i++) {
		frequencyAll[i]=1.0; /*changed by JT*/
		frequencyAve[i]=0.0;
	}
	for(i=0;i<seqNum;i++) {
		for(j=0;j<seqLength;j++) {
			sequenceA[i][j]=aa(sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2]);
			frequency[i][bb(sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2])]++;
			frequencyAll[bb(sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2])]++;
	...
	for(i=0;i<seqNum;i++) {
		...
		for(j=0;j<seqLength;j++) {
			tempr1 = nonsynonymousNew(ratio, KaKsRatio, sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2], frequency[i]);
			tempc1 = synonymousNew(ratio, KaKsRatio, sequenceN[i][j][0],sequenceN[i][j][1],sequenceN[i][j][2], frequency[i]);
			sumw = tempr1+tempc1;
...
if (i == 6 && j == 165) {
         tempr1 = 0;
			...
			/*if (tempr1 < 0.001 || tempc1 < 0.001) printf("%d %f %l", i, tempr1, tempc1);*/
...
printf("%d %d %f\n", i, j, RR[i][j]);
	...
	for(i=0;i<seqNum-1;i++) {
		for(j=i+1;j<seqNum;j++) {
			for (k = 0; k < 70; k++) {
			    frequencyAve[k] = (frequency[i][k]+frequency[j][k])/2.0;
			}
			for(k=0;k<seqLength;k++) {
				round = 0;
				if(sequenceN[i][k][0]==sequenceN[j][k][0]) a1=0;
				...
				if(a4==2) {
					...
					wt[0] = ComputeQab(sequenceN[i][k][0], sequenceN[i][k][1], sequenceN[i][k][2], 
						x1, x2, x3, frequencyAve[bb(x1,x2,x3)], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp);
					...
					wt[1] = ComputeQab(x1,x2,x3,
						...
						frequencyAve[bb(sequenceN[j][k][0],sequenceN[j][k][1],sequenceN[j][k][2])], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp);
				...
				if(a4==3) {
					...
					if(aa(x1,x2,x3)==20||aa(y1,y2,y3)==20) {
						...
						wt[0] = ComputeQab(sequenceN[i][k][0], sequenceN[i][k][1], sequenceN[i][k][2], 
							x1, x2, x3, frequencyAve[bb(x1,x2,x3)], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp)*
							ComputeQab(x1,x2,x3, 
							y1, y2, y3, frequencyAve[bb(y1,y2,y3)], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp)*
							ComputeQab(y1,y2,y3, 
							sequenceN[j][k][0], sequenceN[j][k][1], sequenceN[j][k][2], frequencyAve[bb(sequenceN[j][k][0],sequenceN[j][k][1],sequenceN[j][k][2])], ratio, KaKsRatio, DrDcRatio, indexs, &Synonymousp, &Transversionp, &Radicalp);
					}
					x1=sequenceN[i][k][0];
					x2=sequenceN[i][k][1];
					x3=sequenceN[j][k][2];
...
double radical(double ratio, double KaKsRatio, int u0, int v0, int w0, double* frequencyAll)
{
	...
	for(i=1;i<=3;i++) {
		u=u+1;
		if(u==4) u=0;
		if((u0==0&&u==1)||(u0==1&&u==0)||(u0==2&&u==3)||(u0==3&&u==2)) wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u,v0,w0)]*K1;
		else wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u,v0,w0)]*K2;
		wtcount++;
	}
	...
	for(i=1;i<=3;i++) {
		v=v+1;
		if(v==4) v=0;
		if((v0==0&&v==1)||(v0==1&&v==0)||(v0==2&&v==3)||(v0==3&&v==2)) wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u0,v,w0)]*K1;
		else wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u0,v,w0)]*K2;
	...
	for(i=1;i<=3;i++) {
		w=w+1;
		if(w==4) w=0;
		if((w0==0&&w==1)||(w0==1&&w==0)||(w0==2&&w==3)||(w0==3&&w==2)) wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u0,v0,w)]*K1;
		else wt[wtcount] = frequencyAll[bb(u0,v0,w0)]*frequencyAll[bb(u0,v0,w)]*K2;
