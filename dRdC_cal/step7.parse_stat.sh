...
# the output is ./dRdC_dat/dRdC.Rout.parsed
...
for TARGET in ${TARGET_CLADES[@]} ; do
  ...
  OUTPUT=${WORK_DIR}/dRdC_dat/dRdC.target_${TARGET}.Rout_parsed
  ...
  INPUT=$WORK_DIR/dRdC_dat/target_$TARGET.median.Rinput
  head -1 $OUTPUT > $INPUT  
  for CLASS in 'charge' 'MY' ; do
  ...
  Rscript ${WORK_DIR}/scripts/calc_target_dRdC_avg.R  $INPUT  $OUTPUT
...
cd ${WORK_DIR}/
mkdir -p output
mv dRdC_dat/*.Rout_parsed output
