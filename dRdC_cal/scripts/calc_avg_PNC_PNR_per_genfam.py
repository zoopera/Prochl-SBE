...
if __name__ == "__main__" :
	...
	seqfile = "../../aligned_genes/%s.SEQ" % genfam    
...
#add filter modification, next if target_dC * control_dC == 0
	...
	if len(ls_target_dR) > tgt_cnt*ref_cnt/3  and  len(ls_control_dC) > ctl_cnt*ref_cnt/3:
	    dict_genfam[genfam] = Gene(genfam, ctl_cnt, tgt_cnt, ref_cnt, \
		average(ls_target_dC), average(ls_target_dR), average(ls_control_dC), average(ls_control_dR))
